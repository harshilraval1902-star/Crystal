        ...
        <section className="py-16 relative">
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
              ...
              {/* FORM SECTION */}
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }} className="lg:col-span-7 xl:col-span-8">
                <div className="bg-white rounded-[2rem] border border-gray-100 shadow-xl shadow-primary-900/5 p-8 sm:p-10">
                  <AnimatePresence mode="wait">
                    {submitted ? (
                      <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center py-10">
                        <div className="w-20 h-20 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-6 border border-emerald-100 shadow-sm animate-pulse">
                          <CheckCircle2 className="w-10 h-10" />
                        </div>
                        <h2 className="text-3xl font-extrabold text-gray-900 mb-4 tracking-tight">Booking Sent!</h2>
                        <p className="text-gray-500 mb-8 max-w-sm mx-auto text-lg font-medium">
                          Your service request was routed to WhatsApp. Our team will contact you shortly to confirm timing.
                        </p>
                        <div className="flex flex-col sm:flex-row gap-4 justify-center">
                          <button
                            onClick={() => { setSubmitted(false); setForm(initialForm); }}
                            className="bg-gray-900 text-white font-bold px-8 py-3.5 rounded-full hover:bg-gray-800 transition-all shadow-md hover:scale-\[1.01\]"
                          >
                            Book Another
                          </button>
                          <a
                            href={`tel:${contactNumber}`}
                            className="bg-white border border-gray-200 text-gray-900 font-bold px-8 py-3.5 rounded-full hover:bg-gray-50 hover:border-gray-300 transition-all flex items-center justify-center gap-2 hover:scale-\[1.01\]"
                          >
                      ...
                      <motion.form key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleSubmit} className="space-y-6">
                        <div className="border-b border-gray-100 pb-6 mb-6">
                          ...
                          <p className="text-sm text-gray-500 font-medium">Provide your contact info for the service visit.</p>
                        </div>
                        
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                          <motion.div animate={errors.name ? { x: [-4, 4, -4, 4, 0] } : {}} transition={{ duration: 0.4 }}>
                            ...
                            <input
                              ...
                              className={`w-full bg-gray-50 border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-brand-primary focus:bg-white transition-all ${errors.name ? "border-red-300 focus:ring-red-500/20" : "border-gray-200"}`}
                            />
                            {errors.name && <p className="text-red-500 text-xs font-semibold mt-1.5">{errors.name}</p>}
                          </motion.div>
                          <motion.div animate={errors.phone ? { x: [-4, 4, -4, 4, 0] } : {}} transition={{ duration: 0.4 }}>
                            ...
                            <input
                              ...
                              className={`w-full bg-gray-50 border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-brand-primary focus:bg-white transition-all ${errors.phone ? "border-red-300 focus:ring-red-500/20" : "border-gray-200"}`}
                            />
                            {errors.phone && <p className="text-red-500 text-xs font-semibold mt-1.5">{errors.phone}</p>}
                          </motion.div>
                        </div>

                        <motion.div animate={errors.address ? { x: [-4, 4, -4, 4, 0] } : {}} transition={{ duration: 0.4 }}>
                          ...
                          <input
                            ...
                            className={`w-full bg-gray-50 border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-brand-primary focus:bg-white transition-all ${errors.address ? "border-red-300 focus:ring-red-500/20" : "border-gray-200"}`}
                          />
                        ...
                        <div className="border-t border-gray-100 pt-6 mt-6">
                          ...
                          <p className="text-sm text-gray-500 mb-6 font-medium">What do you need help with?</p>
                        </div>

                        <motion.div animate={errors.serviceType ? { x: [-4, 4, -4, 4, 0] } : {}} transition={{ duration: 0.4 }}>
                          ...
                          <div className="relative">
                            <select
                              ...
                              className={`appearance-none w-full bg-gray-50 border rounded-xl px-4 py-3 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-brand-primary focus:bg-white transition-all ${errors.serviceType ? "border-red-300 focus:ring-red-500/20" : "border-gray-200"}`}
                            >
                          ...
                          {errors.serviceType && <p className="text-red-500 text-xs font-semibold mt-1.5">{errors.serviceType}</p>}
                        </motion.div>
                        ...
                        <div className="pt-4">
                          <button
                            type="submit"
                            disabled={loading}
                            className="w-full flex items-center justify-center gap-2 bg-brand-primary text-white font-bold py-4.5 rounded-full hover:bg-primary-800 transition-all shadow-lg shadow-brand-primary/20 disabled:opacity-70 disabled:cursor-not-allowed hover:scale-\[1.01\] active:translate-y-0 text-base group"
                          >
                            {loading ? (
                              <><Loader2 className="w-5 h-5 animate-spin" /> Processing...</>
                            ) : (
                              <><Send className="w-5 h-5 group-hover:translate-x-1 transition-transform" /> Submit Request via WhatsApp</>
                            )}
                          </button>
                        </div>
                      </motion.form>
              ...
              {/* SIDEBAR */}
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.2 }} className="lg:col-span-5 xl:col-span-4 space-y-6">
                
                {/* Contact Card */}
                <div className="bg-brand-primary rounded-[2rem] p-8 text-white relative overflow-hidden shadow-xl shadow-primary-900/10">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full blur-2xl -mr-10 -mt-10"></div>
                  <h3 className="font-extrabold text-xl mb-2 relative z-10 tracking-tight">Need immediate help?</h3>
                  <p className="text-primary-200 text-sm mb-6 relative z-10 font-medium">Call us directly for urgent repairs or queries.</p>
                  <a
                    href={`tel:${contactNumber}`}
                    className="flex items-center gap-4 bg-white/10 hover:bg-white/20 border border-white/10 p-4 rounded-2xl transition-colors relative z-10 hover:scale-[1.02] duration-300"
                  >
                    <div className="w-12 h-12 bg-white text-brand-primary rounded-full flex items-center justify-center shrink-0 shadow-sm animate-pulse">
                ...
                <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
                  <h3 className="font-extrabold text-gray-900 text-lg mb-6">Service Guarantees</h3>
                  <ul className="space-y-5">
                    <li className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500 leading-relaxed font-semibold">Most service requests are attended to within 24 hours.</div>
                      </div>
                    </li>
                    <li className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500 leading-relaxed font-semibold">We use only original, manufacturer-certified spare parts.</div>
                      </div>
                    </li>
                  </ul>
                </div>
                ...
                <div className="bg-gray-50 rounded-3xl border border-gray-100 p-8">
                  <h3 className="font-extrabold text-gray-900 text-lg mb-4">Working Hours</h3>
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between items-center py-2 border-b border-gray-200/60 font-semibold">
